...
- [x] (2026-08-20) Found that Looking Glass framing was only applied when the polyfill was first constructed, which happens once, so loading a second scene kept the first one's frame. It is now reapplied on every entry, verified by sabotaging a value between entries and watching it be restored.
