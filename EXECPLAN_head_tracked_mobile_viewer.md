...
- Observation: Which processor runs the face model was never stated, and the difference is worth a third of the tracking budget.
  Evidence: `baseOptions` carried only `modelAssetPath`, leaving the delegate to the runtime. The device reports 16 to 17 ms per inference at 20 Hz, which is a third of the 50 ms slot. GPU is now requested explicitly with CPU as a fallback, and the delegate actually in use is shown in the debug readout so the two can be compared on hardware.
