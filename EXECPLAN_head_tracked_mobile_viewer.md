...
- [x] (2026-08-19) Settled the intermittent left/right reversal. The user confirmed `?flip=1`, which is already the default, is correct on their devices, so the inference was never wrong: the setting was being toggled by accident. `Flip L/R` sits in a control bar painted over the canvas, so a finger that is part of a pinch can land on it and activate it on release, and the choice then persists across sessions. Debug mode correlated with the problem only because tuning involves far more touching. Stray taps during a canvas gesture are now ignored, a non-default setting is shown in the signal colour and reads `L/R inverted`, and the debug readout was enlarged from 9px, which was unreadable on a phone.
- [x] (2026-08-19) Stopped Depth Magnification being counted twice on the mobile path. It linearly scales the published depth range and is also carried into the manifest as the relief span, and the first count is not neutral: squeezing the source range toward the near plane shifts the disparity mapping, so a near subject gets a smaller share of a smaller budget. The mobile profile now publishes unscaled depth while the manifest keeps carrying the intent. Far clipping and log shaping still apply, and the desktop, SBS, WebXR and Looking Glass views are unchanged.
- [x] (2026-08-19) Corrected the Looking Glass placement, which had overshot. Anchoring the nearest surface on the focal plane moved the model 2.0 units forward and the user reported it arriving roughly 1.3 to 1.5 too near. The nearest surface now lands on the front face of the hologram volume, at `-targetDiam / 2` read from the live Looking Glass configuration, so the model recedes into the volume instead of protruding through the frame where a Looking Glass clips it. A browser check measured the forward movement dropping from 2.0 to 0.5, and the applied position is reported on entry so it can be corrected against hardware.
- [x] (2026-08-20) Reverted two hand-tuned Looking Glass offsets after they produced no visible change on hardware. Calibrating unverifiable constants against third-hand estimates was the wrong approach; the placement is back to the derived `-targetDiam / 2`, which is where the user's own estimate had put it.
- [x] (2026-08-20) Found why Looking Glass configuration never took effect. `LookingGlassConfig` is a live singleton whose setters drive the renderer, and the code spread it into a plain object and passed that copy to the polyfill constructor, so nothing in it was ever applied. Assign onto the singleton and construct without arguments, per the vendor's documented sequence. Verified in a browser that `?lgTargetY=-0.5&lgTargetDiam=4` now reaches the live config.
- [x] (2026-08-20) Tied the Looking Glass framing reset to loading a different picture. Re-entering with the same picture keeps whatever was just adjusted; a new picture gets the starting frame back, including the values this project does not otherwise override, so nothing a viewer moved can survive into a scene it was not chosen for. Verified across all three cases in a browser.
- [x] (2026-08-20) Settled how Looking Glass framing should be owned. The viewer already adjusts it in the Looking Glass window itself, with the polyfill's own controls plus wheel and drag, and with the editor's Z Offset slider. The app therefore applies its starting values once, when the polyfill is first constructed, and never again, so a manual adjustment survives leaving and re-entering. A panel duplicating those controls was built and then removed as redundant.
- [x] (2026-08-20) Tried and abandoned deriving the Looking Glass frame from the model's bounds. It disagreed with both hardware measurements in magnitude and sign, because the bounding box of a perspective reconstruction is dominated by its far cone rather than by the subject. The defaults are now `targetY 0` and `targetZ -0.575`, and the model's depth placement is a named constant rather than a function of `targetDiam`, so adjusting framing size no longer moves the model in depth.
- [x] (2026-08-20) Exposed the Looking Glass framing controls (`targetX`, `targetY`, `targetZ`, `targetDiam`, `fovy`) on the URL and reported the live values on entry, so framing can be settled against hardware this repository cannot test rather than guessed at.
- [x] (2026-08-19) The user reviewed the tuned presentation on real devices and settled on the shipped defaults: viewing distance from the device table, relief span 1.0, disparity blend 1.0. No default changes were needed; the exploration instead surfaced the radial-depth and double-counted-magnification defects recorded above.
- [x] (2026-08-20) Answered whether the model could instead be left standing in the room and walked around with the phone. Position cannot come from integrating an accelerometer, and the browser exposes no other source of it on iOS, so the honest options are a rotation-only orbit at an assumed radius, or `immersive-ar` where a platform offers it. Added a capability probe to the `?debug=1` readout so the question can be settled on the device rather than argued.
- [x] (2026-08-20) Added gravity levelling, in `webapp/src/device-tilt.js`. The relief shared the screen's up axis, so rolling the device rolled the whole scene with it, which a real object behind glass does not do. Only the screen-plane roll is used, taken from the gravity vector rather than from Euler angles, so it is drift-free and needs no heading. The correction is halved and capped at 18 degrees because the picture has edges that a full counter-rotation would swing into view. Verified numerically that a 20 degree device roll counter-rotates the scene by 10 degrees while the glass-plane point stays exactly put, and that an upright device changes nothing.
- [x] (2026-08-20) Corrected the gravity levelling sign against hardware, which reported the opposite of the shipped default. The physical derivation agrees with the measurement: gravity swinging toward the screen's right edge means the device turned clockwise as the viewer sees it, so the scene must turn anticlockwise, a positive rotation about `+z` in screen coordinates. `?levelFlip=1` remains as the escape hatch, now inverting away from the correct sense.
- [x] (2026-08-20) Confirmed on the user's devices that `immersive-ar` is not available, so no browser-exposed six-degree-of-freedom tracking exists there. Leaving a model standing in the room and walking around it is therefore out of reach on this hardware by any means other than constraining the viewpoint to a sphere and driving it from orientation alone.
- [x] (2026-08-20) Fixed motion access never being asked for in a home-screen app. Starting the camera was awaited before the motion permission was requested, and starting the camera also fetches the face model, so on a cold start the tap's user activation was long gone by the time the request was made. Both permission-gated calls are now issued before anything is awaited. Verified in a browser that a click produces `["camera", "motion"]` before the first microtask, and that the motion answer is recorded even when the camera fails.
- [x] (2026-08-20) Added a `Level` control that appears only when levelling is wanted, the camera is running, and motion access was not granted, since a home-screen app has been seen to skip the prompt entirely and there is otherwise no way to ask again without restarting the camera. It carries the same guard as `Flip L/R` against taps that belong to a canvas gesture.
- [x] (2026-08-21) Cut the mobile viewer's power draw by drawing only when the picture would differ. The eye pose arrives twenty times a second while the animation loop ran at sixty, so two frames in three redrew an identical image, each carrying a quarter of a million vertices, a multisample resolve, and a scan of the relief's front samples. `createRenderGate()` compares every input that moves the picture against what was last drawn, with thresholds set below one pixel of on-screen movement. Measured in a browser: zero draws over two idle seconds where the loop would have made 120, and exactly one draw per input while moving at 20 Hz, against 120.
...
- Observation: A stale browser module cache presents as dead UI controls rather than as an error, and it was hit three times during this work.
  Evidence: The frontend is a set of native ES modules that import each other by name. When a fresh module names an export that a cached sibling does not yet provide, linking fails and the whole graph is abandoned, so `attachUIListeners()` never runs and every button becomes inert. Two automated checks in this session read code that was already on disk and already served correctly, and the user's `Load RGBDE PNG file` button began working immediately after a hard reload. The development host now sends `Cache-Control: no-cache` for everything outside `/viewer-api/`, which costs one conditional request per file.

- Observation (superseded): The relief was said to be measuring depth along the capture ray rather than perpendicular to the image plane. That analysis assumed the mesh unprojected depth correctly, and it did not; see the observation below, which supersedes this one and fixes the real cause.

- Observation: The mesh treated Depth Pro's depth as a distance along the viewing ray when it is a distance along the optical axis, which pulled everything off-axis towards the viewer and tilted upright surfaces.
  Evidence: `depth_pro.py` computes `inverse_depth = canonical_inverse_depth * (W / f_px)` and `depth = 1 / inverse_depth`, which is the pinhole relation between disparity and z, the universal convention for metric monocular depth. `generatePerspectiveMesh` normalised its ray to unit length before multiplying, so a sample landed at `cos(theta)` of its true depth: 3.9 percent short at the top of a 32 degree capture, 9.4 at 50 degrees, 21.5 at the corner of a 65 degree one. A wall at constant depth therefore bowed towards the viewer at its edges, and across a face spanning 10 to 18 degrees the spurious gradient is 3.4 percent of the distance, about 6.8 cm at two metres against a face only 20 cm deep. The user saw this as an upright head rendered as though bowed, stretching when the device was tilted to face the false slope squarely. Storing the unnormalised ray fixes the reconstruction and makes the relief's perpendicular depth reading correct at the same time.
...
- Observation: The intermittent left/right reversal was an accidental setting change, not a tracking defect. The inferred default was correct all along.
  Evidence: The user confirmed that `?flip=1` behaves correctly, and `inferFrontCameraMirrorX` already returns `true`. `Flip L/R` lives in `.viewer-controls`, which is positioned over the canvas at a higher stacking level, so a pointer that is part of a two-finger gesture can land on it and fire a click on release. The stored choice then survives reloads, which is why the reversal appeared to come and go, and why clearing site data appeared to fix it: that reset the stored value back to the correct default. The correlation with debug mode was a correlation with the amount of touching involved in tuning, not with anything debug-specific.

- Observation: The near subject's relief was weaker than it should be because Depth Magnification was applied to the published geometry as well as being carried as the relief span.
  Evidence: For a portrait with the subject at 1.0 to 1.3 m against a wall at 3.0 m, the default magnification of 0.5 published a depth range of 1.0 to 2.0 m rather than 1.0 to 3.0 m. That reduced the subject's share of the relief budget from 34.6 to 26.1 percent on top of halving the span, giving 16 pixels of differential parallax for a 50 mm head movement against 21 pixels once the double count is removed, and 39 pixels at a span of 2.

- Observation: Head-tracked parallax is inherently a much weaker depth cue than the desktop's mouse orbit, independently of any tuning.
  Evidence: A comfortable 50 mm head movement at a 300 mm viewing distance is about 9.5 degrees of viewing angle. A desktop drag rotates the model through tens or hundreds of degrees. On top of that, both eyes see the same image on an ordinary panel, so binocular disparity reports a flat surface at the same time as motion parallax reports depth. Touch rotation, bounded at plus or minus 30 degrees, is the mobile equivalent of the desktop orbit and gives a far stronger cue than head movement alone.

- Observation: A depth relief belongs at the front face of the Looking Glass hologram volume, not on its focal plane.
  Evidence: Anchoring the nearest surface at the focal plane moved the model 2.0 units forward of the monitor placement, and the user reported it arriving 1.3 to 1.5 units too near. Half of the default `targetDiam` of 3 is exactly 1.5. Placing the nearest surface at `-targetDiam / 2` lets the whole relief recede into the volume, so nothing protrudes past the frame, where a Looking Glass clips it; the measured forward movement becomes 0.5. Two further hand-tuned offsets added on top of it produced no visible change on hardware and were reverted.

- Observation: The Looking Glass library's own defaults are not what the placement arithmetic had assumed, and one of them explains the model sitting low.
  Evidence: A browser check of the live configuration reports `targetY 1.6`, `targetDiam 2` and `targetZ -0.5`, against the assumed 0 and 3. `targetY 1.6` is a standing viewer's eye height above the floor of a `local-floor` space, while the model sits near the origin, which is why it framed low. The user independently settled on 0.11 and 0.92 for two scenes, both far below 1.6 and consistent with that reading.

- Observation: Losing the model off screen and carrying stale framing are separate problems with separate fixes.
  Evidence: Rotating past the camera is the model transform in `state.controls`, which is already reset by loading a picture and by double-clicking the canvas, and the double-click still works while a session runs. Framing is the Looking Glass configuration, which has no such reset. Tying its reset to a new picture gives the reset opportunity without discarding a deliberate adjustment, and measurements support it: framing that suited one picture at height 0.11 and size 2.00 suited another at 0.92 and 0.90, so carrying it over is almost always wrong.

- Observation: The framing controls already exist in the Looking Glass window, so the application should provide a starting point and then stay out of the way.
  Evidence: The user fits a scene using wheel and drag on the Looking Glass window, the polyfill's own field-of-view slider, and the editor's Z Offset slider. A panel exposing `targetY`, `targetDiam` and `targetZ` was therefore redundant and was removed. Reapplying configuration on every entry, added earlier to stop a second scene inheriting the first one's frame, was worse than the problem it solved: it silently undid whatever the viewer had just adjusted. Applying once at construction gives a sane starting point while leaving ownership with the viewer, verified by adjusting between sessions and confirming the values survive.

- Observation: Framing values that vary per scene cannot be derived from the model's bounds, for the same reason the mobile relief cannot be fitted on XYZ bounds.
  Evidence: Deriving `targetY` from the transformed bounds' centre gave -0.409 and -0.515 for the two repository fixtures, against measured values of 0.11 and 0.92, and `targetDiam` from the bounding-sphere diameter gave 5.426 and 6.175 against measured 2.00 and 0.90. A perspective reconstruction's bounding box is dominated by the wide far cone, not by the subject. Only `targetZ` generalised, agreeing within 0.03 across two very different scenes.

- Observation: Looking Glass configuration was never being applied at all, which is the more likely reason the framing appeared fixed regardless of what the model did.
  Evidence: `LookingGlassConfig` is a live singleton whose property setters drive the renderer. `ensureLookingGlassPolyfill` spread it into a plain object and passed that copy to the polyfill constructor, which discards the setters, so no configuration in it could take effect. The vendor's documented sequence is to assign onto the singleton and then construct. After the change a browser check confirmed `?lgTargetY=-0.5&lgTargetDiam=4` reaching the live config, alongside the library's own defaults of `targetZ -0.5` and `fovy 0.244`. `targetZ` is not zero, so the hologram volume is not centred on the reference-space origin as the placement arithmetic had assumed.

- Observation: A phone cannot hold a model still in the room from inertial sensing alone, and the limit is not sensor quality.
  Evidence: Position from an accelerometer requires double integration, so error grows as the square of time: at a realistic 2 mg residual bias that is 1 cm after a second, 25 cm after five, and a metre after ten. The dominant term is not bias but gravity leaking through attitude error, where a tenth of a degree of tilt already contributes 17 mg, more than any plausible bias. Orientation is a different matter: pitch and roll are referenced to gravity and do not drift, and yaw is absolute through the magnetometer, though it wanders indoors.

- Observation: The devices in use expose no six-degree-of-freedom tracking to the browser, which settles the question rather than leaving it open.
  Evidence: The capability probe reports `immersive-ar no` on the user's iPhone and iPad, alongside `orientation yes` and `motion yes` behind a permission gate. Visual-inertial tracking is the only thing that holds position over tens of seconds, and it is reachable only through `immersive-ar`; the sensors that are available give orientation, which cannot supply position at all.

- Observation: Two thirds of the mobile viewer's drawing was redundant, and removing it cannot be seen.
  Evidence: `TRACKING_INTERVAL_MS` is 50, so a new eye pose arrives at most twenty times a second, and nothing else changed between them; the continuous animation loop nonetheless redrew sixty times a second. Identical inputs produce identical pixels, so the skipped frames could not have looked different. A browser measurement counted zero draw calls across two idle seconds and one draw per input event while moving, against 120 for the loop in both cases. The thresholds are derived rather than guessed: a picture spanning about 400 CSS pixels for 0.95 world units makes a pixel about 0.0024 units, and each limit is that figure divided by how strongly its input moves the picture, so a suppressed frame is one that could not have shifted a pixel.

- Observation: Comparing against the last drawn frame rather than the last checked one is what keeps a slow movement from being suppressed forever.
  Evidence: A drift of a quarter of the threshold per frame would never trigger if each frame were compared with the previous one, but accumulates against the last drawn frame and crosses it every fourth frame. A regression drives exactly that case and asserts the frame count stays between two and four over twelve frames.

- Observation: Motion access failing only in a home-screen app was the same user-activation race as the Looking Glass first-entry failure, not a separate quirk of standalone mode.
  Evidence: The start handler awaited `tracker.start()` before requesting motion access, and that call fetches the MediaPipe module and face model, which takes seconds when nothing is cached. The transient activation from the tap does not survive it, so the request was made with no activation and never prompted. A home-screen app starts cold every time, which is why it surfaced there while Safari, with a warm cache, happened to stay inside the window. The camera kept working because it was the call that got the activation. Issuing both calls before any await fixes it, and a browser check confirms both are made before the first microtask.

- Observation: The direction a device reports gravity relative to its screen has to be checked against hardware, exactly as the front camera's handedness did.
  Evidence: The shipped levelling sign was the opposite of correct, found by the user through `?levelFlip=1`. The derivation agrees with the measurement once written out: gravity swinging toward the screen's right edge means the device turned clockwise as the viewer sees it, so the scene must turn anticlockwise to stay upright, which is a positive rotation about `+z`. The numeric check now shows a 20 degree clockwise device roll moving the scene's top 10 degrees anticlockwise, with the glass-plane point fixed.

- Observation: There is a drift-free way to deliver most of "walk around it" without position, by constraining the viewpoint instead of measuring it.
  Evidence: If the viewpoint is assumed to lie on a sphere of fixed radius about the model, orientation alone determines it, and orientation does not drift in pitch and roll. That yields an orbit driven by tilting the phone, with no accumulating error. It cannot represent actually walking closer, and it cannot coexist with front-camera tracking, since turning the phone away loses the face, so it belongs as a separate mode rather than as an extension of the window.

- Observation: The weakness the metric head pose was said to leave behind had largely already been fixed by it, and the useful thing an inertial sensor adds is something else.
  Evidence: The recorded limitation was that apparent eye spacing cannot separate a face approaching from a head turning, which applies to the landmark-ratio fallback. The metric path takes distance from the transformation matrix's translation, so head rotation does not corrupt it, and rolling or tilting the device moves the head within the camera frame in a way the off-axis projection already handles correctly, because camera and screen are one rigid body. What remains wrong is that the relief shares the screen's up axis, so the scene rolls with the device; a real object behind glass stays upright while its frame turns.

- Observation: Gravity is the one inertial quantity that can be used without drift, and reading it as a vector avoids a class of platform differences.
  Evidence: Pitch and roll are referenced to gravity and do not accumulate error, unlike a heading, which needs the magnetometer and wanders indoors, and unlike position, which needs double integration. Taking the direction from `accelerationIncludingGravity` rather than from the Euler angles of `deviceorientation` avoids sign conventions that differ between platforms and the degeneracy when a device points straight up or down; the vector case is instead detected and ignored when too little of gravity lies in the screen plane.

- Observation: The render gate's angle threshold was derived for yaw and pitch and wrongly applied to roll as well, which made ordinary hand tremor redraw the view at the motion sensor's rate.
  Evidence: Yaw and pitch swing the deepest geometry, so movement is roughly the relief depth times the angle, and the deepest relief allowed is 8; one pixel is therefore 0.0003 radians. Levelling rotates within the screen plane instead, moving a point by the image radius of about 0.6 times the angle, halved again by the levelling gain, so one pixel is 0.008 radians -- 27 times looser. Holding roll to the depth-derived limit meant almost every `devicemotion` event crossed it. The device reported 35 to 50 frames a second where the pose rate alone predicts 20.

- Observation: Naming the GPU delegate made inference slower, not faster, and the assumption that it would help was wrong.
  Evidence: `baseOptions` originally carried only `modelAssetPath`, and the device measured 16 to 17 ms per inference at 20 Hz. Requesting GPU explicitly raised that to 19.1 to 20 ms, measured while the render gate was drawing 20 to 27 frames a second rather than 35 to 50, so less was competing for the GPU, not more. Forcing CPU afterwards returned 16 to 17 ms, which settles it: the runtime had already chosen CPU, and naming GPU was a regression rather than an optimisation. The face landmarker is a small model, and for small models the delegate's texture upload and readback can cost more than the compute it saves. CPU is tried first now, GPU remains a fallback, and either can be forced from the URL.
...
- Observation: The orientation offset is always a multiple of a quarter turn, so it can be removed by rounding rather than by consulting `screen.orientation.angle` at all.
  Evidence: Three derivations of that angle's convention were tried and all were contradicted by hardware, in different directions on different devices: with the offset left alone, iPhone portrait was level while iPhone landscape tilted left and iPad landscape tilted right; adding it fixed nothing on iPad and broke iPhone portrait; subtracting it made iPhone landscape correct while leaving everything else wrong. A real roll is small — the correction is capped at 18 degrees, and a device held past 45 would have changed orientation — so rounding the measured angle to the nearest quarter turn and subtracting leaves the roll and removes the orientation, whichever convention a platform follows. Verified across 0, 90, 180 and 270 degrees of holding, with and without a real roll on top.
...
- Observation: The relief span settled at one, meaning the relief is as deep as the fitted picture is tall, and the mapping from Depth Magnification is one to one with nothing between them.
  Evidence: Making depth proportional without recalibrating first read as flatter than the previous default, and a 1.5 exaggeration was added to restore parity. With the ceiling then raised to 40 and the range tried on hardware, the user judged 1 the most natural of them, so the exaggeration was removed rather than kept at a value guessed to match old behaviour.
...
- Decision: Derive the Looking Glass placement from the live `targetDiam` rather than a tuned constant, and report the applied position on entry.
  Rationale: The right position follows from the hologram volume's size, which the Looking Glass configuration already states, so reading it adapts automatically if that configuration changes and avoids a magic number calibrated against one device. Reporting the value keeps it correctable against hardware this repository cannot test.
  Date/Author: 2026-08-19 / Claude, prompted by the user's report that the automatic placement overshot

- Decision: Stop calibrating Looking Glass constants against reported impressions, and expose the display's own framing controls instead.
  Rationale: Two hand-tuned offsets were added on the strength of estimates that could not be checked here, and they changed nothing on the device. Where content sits in a Looking Glass frame is the display's business: the polyfill frames a volume of `targetDiam` around `targetX/Y/Z` and renders it with `fovy`. Moving the model only shifts it within a frame that is already fixed. Putting those five values on the URL and reporting them on entry places the tuning where the hardware is.
  Date/Author: 2026-08-20 / Claude, prompted by the user observing no change and inviting a revert

- Decision: Apply gravity levelling partially and with a bound, rather than fully.
  Rationale: A full counter-rotation is what a real window does, but the scene here is a picture with edges, and rotating it fully swings its corners into view. Half the roll, capped at 18 degrees, gives the cue that the object belongs to the room rather than to the screen without exposing the edges. Handedness gets the same escape hatch as the front camera, because which way a device reports gravity relative to its screen is the same kind of device-dependent convention.
  Date/Author: 2026-08-20 / Claude, prompted by the user asking for the window mode's weakness alone to be addressed
