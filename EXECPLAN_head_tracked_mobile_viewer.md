...
- Observation: The relief was measuring depth along the capture ray instead of perpendicular to the image plane, which bows a flat surface backwards at the edges in proportion to the relief depth.
  Evidence: The published mesh is `rayDirection * depth`, so straight-line distance from the camera grows as `1 / cos(theta)`: 16 percent at the corners of the repository's 32 degree fixture, 27 percent for a 50 degree phone capture, 46 percent at 65 degrees. At the shallow spans used earlier this was invisible; at a span of 8 the user reported a cone or dome shape with radial discontinuities, "as if projected onto a small sphere". Because `rayDirection.z` is `-cos(theta)`, the z component already carries the perpendicular depth with that factor divided out.
