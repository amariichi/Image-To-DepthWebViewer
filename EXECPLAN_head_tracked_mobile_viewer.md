...
- [x] (2026-08-19) Applied two hardware-measured Looking Glass corrections: half a world unit of pop-out in front of the hologram volume's front face, and a lift of a quarter of the volume, because the model framed low on a Looking Glass Go. A browser check measured the model moving forward by 1.0 in total and up by 0.75, restoring exactly on exit.
...
- Observation: A depth relief belongs at the front face of the Looking Glass hologram volume, not on its focal plane.
  Evidence: Anchoring the nearest surface at the focal plane moved the model 2.0 units forward of the monitor placement, and the user reported it arriving 1.3 to 1.5 units too near. Half of the default `targetDiam` of 3 is exactly 1.5. Placing the nearest surface at `-targetDiam / 2` lets the whole relief recede into the volume, so nothing protrudes past the frame, where a Looking Glass clips it; the measured forward movement becomes 0.5. On hardware that still read as slightly too deep, and half a unit of pop-out settled it, giving a measured forward movement of 1.0. On the same device the model framed about a quarter of a frame low, corrected by lifting it a quarter of the volume. These two figures are the only values in the Looking Glass path that were measured rather than derived, and both are reported on entry so they stay correctable.
